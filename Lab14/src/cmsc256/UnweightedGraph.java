...
public class UnweightedGraph<V> implements Graph<V> {
	...
	public static UnweightedGraph<Integer> graphFromFile(String fileName) throws FileNotFoundException {
		try {
			...
			UnweightedGraph<Integer> fileGraph = new UnweightedGraph<>();
			while (in.hasNextLine()) {
				String currentLine = in.nextLine();
				String[] lineArray = currentLine.split("\\s+");
				int originVertex = Integer.parseInt(lineArray[0]);
				fileGraph.addVertex(originVertex);
				for (int i = 1; i < lineArray.length; i++) {
					int destinationVertex = Integer.parseInt(lineArray[i]);
					fileGraph.addVertex(destinationVertex);
					fileGraph.addEdge(originVertex, destinationVertex);
				}
			}
			in.close();
			return fileGraph;
		} catch (FileNotFoundException e) {
			throw new FileNotFoundException("File does not exist/open");
		}
	}
	...
	public boolean isComplete() {
		...
		int expectedEdges = numberOfVertices * (numberOfVertices - 1) / 2;
		int actualEdges = 0;
		for (int i = 0; i < numberOfVertices; i++) {
			actualEdges += getDegree(i);
		}
	...
	public boolean areAdjacent(int origin, int destination) {
		...
		List<Integer> neighbors = getNeighbors(origin);
	...
	public boolean isConnected() {
		...
		return tree.getNumberOfVerticesFound() == numVertices;
	}
	...
	public List<V> getShortestPath(int origin, int destination) {
		...
		if (numVertices == 1 && origin == destination) {
			List<V> path = new ArrayList<>();
			path.add(getVertex(origin));
			return path;
		}
		...
		visited[origin] = true;
		int currentIndex = 0;
		while (currentIndex < toProcess.size()) {
			int currentVertex = toProcess.get(currentIndex);
			for (int neighbor : getNeighbors(currentVertex)) {
				if (!visited[neighbor]) {
					visited[neighbor] = true;
					...
					if (neighbor == destination) {
						...
						while (pathVertex != -1) {
							path.add(getVertex(pathVertex));
							pathVertex = parent[pathVertex];
						}
						List<V> reversedPath = new ArrayList<>(path.size());
						for (int i = path.size() - 1; i >= 0; i--) {
